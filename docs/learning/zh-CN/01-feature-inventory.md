...
| Web Dashboard | 浏览器 Native Chat，配合会话、配置、日志和辅助面板。 | `hermes dashboard`；非 loopback 绑定必须配置认证。 | `/chat` 通过 `/api/ws` 连接 tui_gateway、AIAgent 和 SessionDB；插件覆盖优先。 | [`web/src/pages/ChatPage.tsx`](../../../web/src/pages/ChatPage.tsx) | [Web Dashboard](../../../website/docs/user-guide/features/web-dashboard.md) | 核心客户端；Agent terminal 工具与独立 TUI 保留。 |
...
| Dashboard Auth | 保护非 loopback HTTP、REST、WebSocket。 | Auth Provider/`dashboard register`。 | HTTP 与 WS 共用 Auth Policy；公开绑定没有 Insecure Bypass。 | [`plugins/dashboard_auth/`](../../../plugins/dashboard_auth/) | [Dashboard](../../../website/docs/user-guide/features/web-dashboard.md) | 公开部署强制认证。 |
