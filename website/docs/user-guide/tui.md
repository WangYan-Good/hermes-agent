...
The default standalone TUI uses stdio. `HERMES_TUI_GATEWAY_URL` is internal WebSocket transport wiring for clients attaching to an authenticated Hermes backend. Dashboard Native Chat uses `/api/ws` directly and does not launch Ink or a PTY child.
...
If you want multiple surfaces to share one set of sessions, use the shared `~/.hermes/state.db` (see [Sessions](sessions.md)) or the web dashboard's Native Chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.
