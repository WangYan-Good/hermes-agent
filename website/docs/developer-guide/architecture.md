...
Native is the sole built-in Dashboard chat surface over `/api/ws`. Agent terminal
tools, standalone CLI/TUI and Desktop terminal features remain supported.
Dashboard presentation does not alter tool schemas, prompt caching or SessionDB.
...
Plugin manifests must be freshly confirmed before built-in chat can mount.
...
Durable conversations created by the former Terminal surface are ordinary
SessionDB sessions: Native resumes the canonical durable ID through the existing
compression/branch resolution, without cloning or conversion.

Reconnect, refresh and ambiguous submission recover through existing
resume/activate and attachment-ledger paths, never by repeating `prompt.submit`.
Native resume explicitly passes `allow_auto_continue=false`, including deferred
history hydration. Other clients retain their normal continuation policy. Worker
finalization, queued prompts, goals and compute-host completion retain their
existing ordering; Native observes ordinary message/session events rather than
a presentation-control protocol.
