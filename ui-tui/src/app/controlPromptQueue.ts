...
type ControlResult =
  | { kind: 'expire'; promptKind: 'clarify' | 'secret' | 'sudo'; requestId: string }
  | { kind: 'request'; prompt: ControlPrompt }

const current = (): ControlPrompt | null => {
  const state = getOverlayState()

  if (state.approval) {
    return { kind: 'approval', request: state.approval }
  }

  if (state.clarify) {
    return { kind: 'clarify', request: state.clarify }
  }

  if (state.secret) {
    return { kind: 'secret', request: state.secret }
  }

  if (state.sudo) {
    return { kind: 'sudo', request: state.sudo }
  }
...
const matches = (prompt: ControlPrompt, kind: ControlKind, requestId: string | undefined, sessionId: string) =>
  prompt.kind === kind &&
  prompt.request.sessionId === sessionId &&
  (requestId === undefined || idOf(prompt) === requestId)
...
const promote = () => {
  ...
  if (next) {
    patchOverlayState({ [next.kind]: next.request, controlQueue })
  }
}
...
export const completeControlPrompt = (kind: ControlKind, requestId: string | undefined, sessionId: string) => {
  ...
  patchOverlayState(state => ({
    ...state,
    controlQueue: state.controlQueue.filter(prompt => !matches(prompt, kind, requestId, sessionId))
  }))
}
...
export const controlPromptFromEvent = (
  ...
  switch (event.type) {
      ...
      return {
        ...
        prompt: {
          ...
          request: {
            ...
            description: String(event.payload.description ?? 'dangerous command'),
            sessionId,
            sessionTitle,
            smartDenied: event.payload.smart_denied === true
          }
        }
      }
    ...
    case 'sudo.request':
      return {
        kind: 'request',
        prompt: { kind: 'sudo', request: { requestId: event.payload.request_id, sessionId, sessionTitle } }
      }
