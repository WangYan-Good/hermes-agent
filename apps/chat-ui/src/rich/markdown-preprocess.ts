...
export interface MarkdownTransforms { stripPreviewTargets?: (text: string) => string; linkifySessionRefs?: (text: string) => string }
...
function normalizeVisibleProse(text: string, linkifySessionRefs = identity): string {
  return text
...
export function preprocessMarkdown(text: string, transforms: MarkdownTransforms = {}): string {
  const stripPreviewTargets = transforms.stripPreviewTargets ?? ((value: string) => value.trim())
  ...
  return strippedEmptyFences
    ...
    .map(part => {
      ...
      const transformed = clampHtmlNestingDepth(normalizeVisibleProse(stripPreviewTargets(normalizeProseMath(part)), transforms.linkifySessionRefs))
