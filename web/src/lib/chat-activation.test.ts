...
describe("latchChatActivation", () => {
  it("stays inactive while the chat tab has never been active", () => {
    ...
    // so the persistent Native host does not open an Agent connection (which would
    // trigger the Agent bootstrap on every page).
  ...
  it("stays activated after the chat tab is left (sticky / persistence)", () => {
    // Once the user has opened /chat, the Native host must survive navigating away so
