declare global {
  interface Window {
    /**
     * Injected by the server as `true`. The Native Chat surface
     * (`/chat`, `/api/ws`) is always enabled, so this is
...
/**
 * Whether the dashboard's Native Chat surface is available.
 *
 * The embedded chat (`/chat` tab, `/api/ws` WebSockets) is now
