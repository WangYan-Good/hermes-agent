...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    def _run():
        with _profile_scope(profile):
            # Read once for the whole list. A broken config must not take down
            # the Accounts tab, so every row falls back to an unconfigured
            # proxy state when config cannot be read.
            try:
                proxy_cfg = load_config()
            except Exception:
                _log.warning(
                    "oauth list: config unreadable, reporting no proxies",
                    exc_info=True,
                )
                proxy_cfg = {}
            providers = []
            for p in _build_oauth_catalog():
                ...
                providers.append({
                    ...
                    # claude-code subscription row), which is how the UI knows
                    # to render no proxy editor.
    ...
    def _run():
        with _profile_scope(profile):
            with _CONFIG_MUTATION_LOCK:
                ...
                try:
                    state = apply_provider_proxy(
                        cfg, provider_id, body.mode, body.url
                    )
                except ValueError as exc:
                    # Already credential-safe: the coercion layer redacts
...
def _resolve_chat_argv(
    ...
    # Browser chat uses a fixed alternate-screen viewport. Ink owns the
    # transcript/output ScrollBoxes, so forward wheel events to it without
    # enabling click/drag/hover terminal tracking.
